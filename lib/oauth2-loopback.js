...
module.exports = function(app, options) {
  ...
  function createAccessToken(client, user, scope, cb) {
    ...
    debug('Generating access token: %s %s %s %s',
      token, clientInfo(client), user.username, scope);
    ...
    server.exchange(oauth2Provider.exchange.password(
      function(client, username, password, scope, done) {
        ...
        userLogin(username, password, function(err, user) {
          ...
          if (typeof options.passwordIssue === 'function') {
            options.passwordIssue(client, user, scope, function(err, client, user, scope) {
              if (err) {
                return done(err, null);
              }
              createAccessToken(client, user, scope, done);
            })
          }
          else {
            createAccessToken(client, user, scope, done);
